...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	XKProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	XKProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
		return YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [XKProgressHUD allHUDsForView:view];
	for (XKProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[XKRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		[indicator removeFromSuperview];
		self.indicator = MB_AUTORELEASE([[HFTBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[XKRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(XKRoundProgressView *)indicator setAnnular:YES];
		}
		[(XKRoundProgressView *)indicator setProgressTintColor:self.activityIndicatorColor];
		[(XKRoundProgressView *)indicator setBackgroundTintColor:[self.activityIndicatorColor colorWithAlphaComponent:0.1f]];
	}
...
- (void)layoutSubviews {
	...
	CGSize labelSize =  MB_MULTILINE_TEXTSIZE(label.text, label.font, CGSizeMake(maxWidth, CGFLOAT_MAX), label.lineBreakMode);
...
@implementation XKRoundProgressView
...
@implementation HFTBarProgressView
