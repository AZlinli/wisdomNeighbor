//
//  XKIMInputEmoticonParser.m
//  XKSquare
//
//  Created by william on 2018/12/10.
//  Copyright © 2018 xk. All rights reserved.
//

#import "XKIMInputEmoticonParser.h"
#import "XKEmotionKeyBoradManager.h"
@implementation XKIMInputTextToken
@end
@interface XKIMInputEmoticonParser()
@property (nonatomic,strong)    NSCache *tokens;
@end

@implementation XKIMInputEmoticonParser
+ (instancetype)currentParser
{
    static XKIMInputEmoticonParser *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[XKIMInputEmoticonParser alloc] init];
    });
    return instance;
}

- (instancetype)init
{
    if (self = [super init])
    {
        _tokens = [[NSCache alloc] init];
    }
    return self;
}

- (NSArray *)tokens:(NSString *)text
{
    NSArray *tokens = nil;
    if ([text length])
    {
        tokens = [_tokens objectForKey:text];
        if (tokens == nil)
        {
            tokens = [self parseToken:text];
            [_tokens setObject:tokens
                        forKey:text];
        }
    }
    return tokens;
}

- (NSArray *)parseToken:(NSString *)text
{
    NSMutableArray *tokens = [NSMutableArray array];
    static NSRegularExpression *exp;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        exp = [NSRegularExpression regularExpressionWithPattern:@"\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]"
                                                        options:NSRegularExpressionCaseInsensitive
                                                          error:nil];
    });
    __block NSInteger index = 0;
    [exp enumerateMatchesInString:text
                          options:0
                            range:NSMakeRange(0, [text length])
                       usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
                           NSString *rangeText = [text substringWithRange:result.range];
                           if ([self isHasEmotion:text])
                           {
                               if (result.range.location > index)
                               {
                                   NSString *rawText = [text substringWithRange:NSMakeRange(index, result.range.location - index)];
                                   XKIMInputTextToken *token = [[XKIMInputTextToken alloc] init];
                                   token.type = XKIMInputTokenTypeText;
                                   token.text = rawText;
                                   [tokens addObject:token];
                               }
                               XKIMInputTextToken *token = [[XKIMInputTextToken alloc] init];
                               token.type = XKIMInputTokenTypeEmoticon;
                               token.text = rangeText;
                               [tokens addObject:token];
                               
                               index = result.range.location + result.range.length;
                           }
                       }];
    
    if (index < [text length])
    {
        NSString *rawText = [text substringWithRange:NSMakeRange(index, [text length] - index)];
        XKIMInputTextToken *token = [[XKIMInputTextToken alloc] init];
        token.type = XKIMInputTokenTypeText;
        token.text = rawText;
        [tokens addObject:token];
    }
    return tokens;
}

-(BOOL)isHasEmotion:(NSString *)text{
    for (XKSticker *sticker in [XKEmotionKeyBoradManager sharedInstance].allStickers) {
        if ([text containsString:sticker.Desc]) {
            return YES;
        }
    }
    return NO;
}

@end
