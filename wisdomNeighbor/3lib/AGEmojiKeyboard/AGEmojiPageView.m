...
- (void)setButtonImages:(NSArray *)buttonStickers{
    NSString *path = [NSBundle.mainBundle pathForResource:@"XKEmotionResource" ofType:@"bundle"];
    if (([self.buttons count] - 1) == [buttonStickers count]) {
        // just reset text on each button
        for (NSUInteger i = 0; i < [buttonStickers count]; ++i) {
            XKSticker *sticker = buttonStickers[i];
            [self.buttons[i] setImage:[UIImage imageWithContentsOfFile:[path stringByAppendingPathComponent:sticker.Image]] forState:UIControlStateNormal];
        ...
        for (NSUInteger i = 0; i < [buttonStickers count]; ++i) {
            UIButton *button = [self createButtonAtIndex:i];
            XKSticker *sticker = buttonStickers[i];
            [button setImage:[UIImage imageWithContentsOfFile:[path stringByAppendingPathComponent:sticker.Image]] forState:UIControlStateNormal];
            NSInteger number = 0;
            for (XKSticker *stickerOne in [XKEmotionKeyBoradManager sharedInstance].allStickers) {
                if ([sticker.Desc isEqualToString:stickerOne.Desc]) {
                    number = [[XKEmotionKeyBoradManager sharedInstance].allStickers indexOfObject:stickerOne];
                }
            }
            button.tag = number + 200;
            [self addToViewButton:button];
        }
        UIButton *button = [self createButtonAtIndex:self.rows * self.columns - 1];
        [button setImage:self.backSpaceButtonImage forState:UIControlStateNormal];
...
- (void)emojiButtonPressed:(UIButton *)button {
    ...
    [self.delegate emojiPageView:self didUseEmoji:(int)button.tag];
}
