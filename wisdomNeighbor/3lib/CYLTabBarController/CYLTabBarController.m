...
static BOOL avoidRepeatFlag = YES;
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (no_Apper) {
        for (UIViewController *vc in self.viewControllers) {
            ...
            [vc.tabBarItem.cyl_tabButton cyl_setTabBadgePointView:tabBadgePointView];
...
//    if ([XKUserDefault boolForKey:isTourist]) {
//        if (self.selectedIndex > 0) {
//            [XKGlobleCommonTool gotoLogin];
//            return;
//        }
//    }
    SEL actin = @selector(tabBarController:didSelectControl:);
    [self oglFlipAnimation:control];
